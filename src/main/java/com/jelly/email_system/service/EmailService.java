...
public class EmailService {
    ...
    public String mudarStatus(StatusRequestDTO status) {
    	...
    	EmailStatusId idComposto = new EmailStatusId(
    	        status.idStatus().idUsuario(), 
    	        status.idStatus().idEmail()
    	    );
    	
    	EmailStatus entidade = emailStatusRepository.findById(idComposto)
    			.orElseThrow(() -> new RuntimeException("Objeto não encontrado com o ID: " + status.idStatus()));
    	try {
    		Status novoStatus = Status.valueOf(status.statusEmail().toUpperCase());
    		
    		entidade.setStatus(novoStatus);
    		
    		emailStatusRepository.save(entidade);
    		
    		return "atualizando do status concluida";
